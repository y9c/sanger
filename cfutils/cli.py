...
@cli.command(
    ...
    no_args_is_help=True, context_settings=CTX,
)
...
@click.option("--aligned/--mutated", default=False,
...
def mut(query, subject, outdir, outbase, aligned, plot):
    ...
    report_mutation(
        query_ab1_file=query, subject_fasta_file=subject, output_dir=outdir,
        file_basename=outbase, report_all_sites=aligned, report_mut_plot=plot,
    )


@cli.command(
    help="Report per-read QC metrics as a table.", no_args_is_help=True,
    context_settings=CTX,
)
...
def qc(files):
    ...
    for f in files:
        ...
        click.echo("\t".join(str(x) for x in [
            Path(f).name, int(m["n_bases"]), f"{m['gc_percent']:.1f}",
...
def track_join(files, outbase, outdir, gap):
    ...
    from cfutils.tracks import join_tracks, export_tracks
...
@click.option("--cuts", "-c", required=True,
              help="Comma-separated 1-based cut positions")
...
def track_split(file, cuts, outdir, fmt):
    ...
    from cfutils.tracks import split_track, export_tracks
...
def track_slice(file, start, end, outdir, fmt):
    ...
    from cfutils.tracks import slice_track, export_tracks
...
def edit_strip_primers(file, forward, reverse, outdir):
    ...
    _emit(strip_primers(__parse_abi(file), forward=forward, reverse=reverse),
          outdir)
...
@click.argument("file", type=click.Path(exists=True))
@click.option("--hetero-threshold", "-r", default=0.45, type=float,
              help="second-peak ratio for ambiguity calling (0 disables)")
@click.option("--outdir", "-o", default=".")
def basecall_call(file, hetero, outdir):
    ...
    res = call_bases(rec, hetero_threshold=hetero)
...
def basecall_hetero(file, min_ratio):
    ...
    for c in res.calls:
        if c.is_ambiguous:
            click.echo(f"{c.position}\t{c.base}\t{c.second_base}"
...
def assemble_consensus(reads, reference, min_cov, min_freq, quality):
    ...
    table = pileup([__parse_abi(f) for f in reads], __parse_fasta(reference),
                   quality_threshold=quality, min_cov=min_cov)
...
def analyze_motif(file, motif, both_strands):
    ...
    click.echo("\t".join(map(str, find_motifs(
        __parse_abi(file).seq, motif, both_strands=both_strands))))
...
def export_vcf(query, subject, min_qual):
    ...
    from pathlib import Path
    sites = call_mutations(__parse_abi(query), __parse_fasta(subject),
                           report_all_sites=True)
    click.echo(to_vcf(sites, reference_name=Path(subject).stem,
                      min_qual=min_qual), nl=False)
...
def export_batch(files, outdir, fmt):
    ...
    click.echo("✓ wrote " + write_batch([__parse_abi(f) for f in files],
                                        outdir, fmt=fmt))
