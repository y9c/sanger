...
Runs the whole sanger pipeline on ``data/B5-M13R_B07.ab1`` vs
...
from sanger import style as _style
...
def fig_composite(rec, out):
    ...
    ax_chrom.set_title(
        "Chromatogram + influence panels (Side-by-side / composite)", loc="left"
    )
    fig.savefig(out)
    plt.close(fig)
...
def fig_feature_overlay(rec, mutations, out):
    ...
    for mut in mutations[:4]:
        feats.append(ChromatogramFeature.from_sitepair(mut, color="#d62728"))
...
def fig_lollipop(rec, mutation_snps, all_sites, out):
    ...
    ax.set_title("SNP lollipop: high-quality variants across the read", loc="left")
...
def fig_assembly(rec, ref, out):
    ...
    ax1.set_title("Reference-guided pileup & consensus (assembly module)", loc="left")
    ...
    ax2.bar(np.arange(len(cons_b)), np.ones(len(cons_b)), color=colors, width=1)
    for i, b in enumerate(cons_b):
        ax2.text(i, 0.5, b, ha="center", va="center", fontsize=6, family="monospace")
    ax2.set_yticks([])
...
def fig_dna_viewer(rec, snps, out):
    ...
    from sanger.show import center_region
    ...
    ax_feat.set_title(
        "DNA Features Viewer map + sanger chromatogram (combined)", loc="left"
    )
    fig.tight_layout()
    fig.savefig(out)
    plt.close(fig)
...
def fig_basecaller_compare(rec, out):
    ...
    from sanger.parser import parse_abi as _p
    ...
    ax.plot(x, mine[:n], color="#d62728", lw=1.0, alpha=0.8, label="sanger re-call")
    ax.axhline(20, color="orange", ls="--", lw=1)
    ax.set_xlabel("Read position")
    ax.set_ylabel("Phred quality")
    ax.set_title("Vendor quality vs sanger base-caller quality", loc="left")
    ax.legend(loc="upper right")
    ax.grid(alpha=0.3)
    fig.savefig(out)
    plt.close(fig)
...
def fig_base_composition(rec, out):
    ...
    labels = ["A", "C", "G", "T"]
    vals = [freq[label] for label in labels]
...
This page walks through the full sanger pipeline on the bundled real data
...
this is how external tools can plot their own signal alongside sanger.
...
def write_html(context, path, figure_paths):
    ...
    labels = [
        ...
        ("basecall", "Vendor quality vs sanger base-caller quality"),
    ]
...
<title>sanger — real-data analysis report</title>
<style>
...
<h1>sanger · real-data analysis report</h1>
...
def main() -> None:
    ...
    snps = [
        s for s in raw_mut if not {"N", "-"} & {s.ref_base, s.cf_base} and qf.passed(s)
    ]
    ...
    fig_chromatogram_mutations(
        rec,
        raw_mut,
        center_region(snps[0].cf_pos, 20, len(rec)),
        figs["chrom_at601"],
        "SNPs ref60_61 highlighted",
    )
    fig_chromatogram_mutations(
        rec,
        raw_mut,
        center_region(215, 20, len(rec)),
        figs["chrom_cluster"],
        "SNP cluster ref169-177",
    )
    ...
    from sanger.basecaller import basecaller_score, call_bases
    from sanger.parser import parse_abi as _parse_abi_raw
